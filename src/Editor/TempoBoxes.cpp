...
TempoBoxesImpl()
{
	myBoxBar.texture = myBoxHl.texture = Texture("assets/icons tempo.png", false);
...
int selectSegments(const Tempo* tempo)
{
	...
	for(auto list = segments->begin(), listEnd = segments->end(); list != listEnd; ++list)
	{
		...
		for(auto seg = list->begin(), segEnd = list->end(); seg != segEnd; ++seg)
		{
			while(box != boxEnd && (box->type != type || box->row < seg->row))
			{
				++box;
			}
...
int selectTime(SelectModifier mod, double begin, double end, int xl, int xr)
{
	auto coords = gView->getNotefieldCoords();
	const int baseX[2] = {coords.xl, coords.xr};
...
void tick()
{
	...
	if(!GuiMain::isCapturingMouse())
	{
		...
		for(int i = 0; i < myBoxes.size(); ++i)
		{
			...
			if(IsInside(recti{x, y - 16, (int)box.width, 32}, mpos.x, mpos.y))
			{
				myMouseOverBox = i;
				break;
			}
...
void draw()
{
	...
	if(myMouseOverBox >= 0 && myShowHelp && !gSystem->isMouseDown(Mouse::LMB))
	{
